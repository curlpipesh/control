...
public class CommandAudit extends CCommand {
    ...
    public boolean onCommand(final CommandSender commandSender, final Command command, final String s, final String[] args) {
        if(args.length == 1) {
            ...
            final Optional<AudreyUser> audreyUser = Users.getInstance().getAudreyUserMap().getUserByName(playerName);
            if(audreyUser.isPresent()) {
                List<Punishment> active = new ArrayList<>(getControl().getActivePunishments()
                        .getPunishmentsBy(audreyUser.get().getUuid().toString()));
                ...
                if(!active.isEmpty() || !inactive.isEmpty()) {
                    commandSender.sendMessage("§a" + audreyUser.get().getLastName() + "§7's stats:");
                    ...
                    commandSender.sendMessage("§a" + audreyUser.get().getLastName() + "§7 has §c" +
                            (active.size() + inactive.size()) + "§7 total punishments");
                    commandSender.sendMessage("§7§m------------------------------------§7");
                } else {
                    commandSender.sendMessage("§a" + audreyUser.get().getLastName() + "§7 has no issued punishments!");
                }
                return true;
            } else {
