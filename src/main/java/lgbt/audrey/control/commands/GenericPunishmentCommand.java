...
public class GenericPunishmentCommand extends CCommand {
    ...
    public boolean onCommand(final CommandSender commandSender, final Command command, final String s, final String[] args) {
        ...
        if(args.length >= 1) {
            ...
            final Optional<AudreyUser> audreyUser;
            ...
            audreyUser = Users.getInstance().getAudreyUserMap().getUserByName(target);
            if(audreyUser.isPresent()) {
                ...
                // old target's IP
                if(punishIP) {
                    finalTarget = audreyUser.get().getIp().toString().replaceAll("/", "");
                } else {
                    finalTarget = audreyUser.get().getUuid().toString();
                }

                if(isUndo) {
                    unpunish(finalTarget, commandSender, audreyUser.get());
                    return true;
                } else {
                    ...
                    if(type == PunishmentType.BAN) {
                        kickForBan(Bukkit.getPlayer(audreyUser.get().getUuid()), formatBan(reason,
                                t ? TimeUtil.english(args[1]) : "Forever",
                                p.isPresent() ? new Date(p.get().getEnd()).toString() : "Some point in the future"),
                                () -> Bukkit.getOfflinePlayer(audreyUser.get().getUuid()).isOnline());
                    } else if(punishIP) {
                    ...
                    announcePunishment(commandSender.getName(), punishIP ? hideIP(finalTarget) : audreyUser.get().getLastName(), type, reason, t ? args[1] : "" + time);
                }
     ...
     * @param audreyUser    TODO
     */
    private void unpunish(final String target, final CommandSender commandSender, final AudreyUser audreyUser) {
        ...
        if(activePunishments.isEmpty()) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(getControl(), () -> {
                ...
                getControl().sendMessage(commandSender,
                        formatUnpunish(punishIP ? hideIP(p.getTarget()) : audreyUser != null ? audreyUser.getLastName()
                                : target, p.getType()));
            }, 0L);
        }
    }
